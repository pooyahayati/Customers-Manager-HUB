...
if marker not in content:
    raise SystemExit("run snapshot marker not found")
lease_helpers = '''
    ...
    try:
        ...
        if not generation_task.done():
            ...
            try:
                ...
                pass
'''
content = content.replace(marker, lease_helpers + marker, 1)
old_signature = '''async def process_agent_event(
    session_factory: AsyncSessionFactory,
    ai_gateway: AIGateway,
...
'''
new_signature = '''async def process_agent_event(
    session_factory: AsyncSessionFactory,
    ai_gateway: AIGateway,
    channel_registry: ChannelRegistry,
    settings: Settings,
...
if old_signature not in content:
    raise SystemExit("process_agent_event signature not found")
content = content.replace(old_signature, new_signature, 1)
old_call = '''                result = await ai_gateway.generate(
                    snapshot.tenant_id,
                    AITaskType.CUSTOMER_RESPONSE,
                    request,
                )
'''
new_call = '''                result = await _generate_with_lease_renewal(
                    session_factory,
                    claim,
                    ai_gateway,
...
if old_call not in content:
    raise SystemExit("AI generation call not found")
...
'''
import_replacement = import_marker + '''from customers_manager_hub.agent_runtime import (
...
class_marker = '''class RecordingMockAIProvider(MockAIProviderAdapter):
'''
if class_marker not in content:
    raise SystemExit("recording provider marker not found")
# Insert the observing subclass after the complete RecordingMockAIProvider class by locating the fixture.
fixture_marker = '''
